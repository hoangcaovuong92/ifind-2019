	...
	function ifind_business_tab_script() {
		jQuery(document).ready(function () {
			...
			jQuery('.business-back-link').on('click', function (e) {
				e.preventDefault();
				ifind_debug_mode('reset system (back link)!');
				ifind_reset_system();
			});
...
if (typeof ifind_is_fancybox_opened != 'function') {
	function ifind_is_fancybox_opened() {
		return jQuery('.fancybox-opened').length == 0 ? false : true;
	}
}

if (typeof ifind_is_fancybox_auto_close != 'function') {
	function ifind_is_fancybox_auto_close(time, position_fancybox) {
		var timer;
		jQuery('.fancybox-wrap').on('mousemove touchstart click', function (event) {
			event.preventDefault();
			if (timer) clearTimeout(timer);
			timer = setTimeout(function () {
				if (!ifind_is_fancybox_opened()) {
					ifind_debug_mode('ignore popup auto close!');
					return;
				}
				ifind_fancybox_close();
				ifind_reset_system();
				ifind_debug_mode('reseted system ('+position_fancybox+')!');
			}, time);
		});
	}
}
...
if (typeof ifind_fancybox_script != 'function') {
	function ifind_fancybox_script() {
		...
		jQuery(".ifind-fancybox-image").fancybox({
			...
			afterClose: function () {},
			afterLoad: function () {
				ifind_is_fancybox_auto_close(short_waiting_time, 'fancybox-image');
			}
		});
		...
		jQuery(".ifind-fancybox-content-file").fancybox({
			...
			afterClose: function () {},
			afterLoad: function () {
				ifind_is_fancybox_auto_close(short_waiting_time, 'fancybox-content-file');
			}
		});
		...
		jQuery('.map-directions-email-send').on('click', function (e) {
			...
			jQuery.fancybox('#map-directions-email', {
				...
				afterLoad: function () {
					ifind_is_fancybox_auto_close(long_waiting_time, 'directions-email');
				}
			});
		});
		...
		jQuery('.fancybox-contact-us').on('click', function (e) {
			...
			if (business_email && href) {
				...
				jQuery.fancybox({
					...
					afterClose: function () {},
					afterLoad: function () {
						ifind_is_fancybox_auto_close(long_waiting_time, 'contact-us');
					}
				});
			}else{
				swal({
...
if (typeof ifind_disabled_browser_action != 'function') {
	function ifind_disabled_browser_action() {
		//Disable Right Click
